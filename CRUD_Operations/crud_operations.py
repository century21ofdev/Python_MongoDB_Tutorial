...
crud_obj = CrudOperations(uri="mongodb://localhost:27017/", client='bookshop')  # firstly make an instance

crud_obj.create(collection='books', inserted_value={'BookName': 'BookAuthor'})  # single insertion
crud_obj.create(collection='books',
                inserted_value=[{'XBookName': 'YAuthor'}, {'YBookName': 'YAuthor'}, {'ZBookName': 'ZAuthor'}],
                multiple_insert=True)  # multiple insertion

crud_obj.retrieve(collection='books')

crud_obj.update(collection='books', query={'YBookName': 'YAuthor'},
                updated_value={"$set": {'YBookName': 'Century21ofDev'}})  # single update

crud_obj.delete(collection='books', query={'ZBookName': 'ZAuthor'})  # single deletion
